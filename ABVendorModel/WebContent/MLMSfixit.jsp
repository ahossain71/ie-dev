...
<body>
	<form name="fixVendorEnrollments" method="POST">
		<%
			...
			if (userName == null || userName.length() == 0 || guid == null
					|| guid.length() == 0) {
		...
		<table>
			<tr>
				...
				<td><input type="text" name="<%=PARAM_USERNAME%>"
					value="<%=userName%>" style="width: 300px;" /></td>
				<td>Enter User's GUID :</td>
				<td><input type="text" name="<%=PARAM_GUID%>" value="<%=guid%>"
		...
		<%
				...
				if (locator != null) {
					...
					if (employee != null) {
						out.println("<p>Display Name " + employee.getId()
								+ "<br></p>");
		...
		<p align="center">
			<input type="hidden" name="<%=PARAM_ACTION%>" value="ACTION" /><input
...
<p align="center"><a  href="MLMSfixit.jsp">Return to Form</a></p>
<% } %>
<footer>
